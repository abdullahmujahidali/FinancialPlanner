...
async function main() {
  ...
  if (existing.length) { console.log("Seed already ran (owner exists). Nothing to do."); return; }

  const [owner] = await db.insert(t.users).values({
    email: OWNER_EMAIL, name: "Abdullah", passwordHash: await bcrypt.hash(OWNER_PASS, 10)
  }).returning();
  const [member] = await db.insert(t.users).values({
    email: MEMBER_EMAIL, name: "Tooba", passwordHash: await bcrypt.hash(MEMBER_PASS, 10)
  }).returning();

  const [hh] = await db.insert(t.households).values({
    name: "Mujahid family", monthlyBudget: "350000", incentivePct: 10
  }).returning();
  ...
  await db.insert(t.persons).values(["Abdullah", "Tooba", "Miral", "Haroon"].map(name => ({ householdId: hh.id, name })));

  const accounts = await db.insert(t.accounts).values([
  ...
  const cats = await db.insert(t.categories).values([
  ...
  await db.insert(t.importRules).values([
    { householdId: hh.id, pattern: "LESCO", setCategoryId: cat("Utilities"), setPassthrough: true, priority: 10 },
    { householdId: hh.id, pattern: "PTCL", setCategoryId: cat("Utilities"), setPassthrough: true, priority: 10 },
    { householdId: hh.id, pattern: "SNGPL", setCategoryId: cat("Utilities"), setPassthrough: true, priority: 10 },
    { householdId: hh.id, pattern: "1BILL", setCategoryId: cat("Utilities"), setPassthrough: true, priority: 20 },
    { householdId: hh.id, pattern: "ZONG", setCategoryId: cat("Mobile & internet"), priority: 30 },
    { householdId: hh.id, pattern: "JAZZ", setCategoryId: cat("Mobile & internet"), priority: 30 },
    { householdId: hh.id, pattern: "UFONE", setCategoryId: cat("Mobile & internet"), priority: 30 },
    { householdId: hh.id, pattern: "LAHORE GRAMMAR SCHOOL", setCategoryId: cat("Kids' education"), priority: 40 },
    { householdId: hh.id, pattern: "MUJAHID ALI", setCategoryId: cat("Family support"), priority: 40 },
    { householdId: hh.id, pattern: "MCDONALDS", setCategoryId: cat("Groceries & food"), priority: 60 }
  ] as any);

  console.log("Seeded. Sign in as:");
  console.log(`  owner:  ${OWNER_EMAIL} / ${OWNER_PASS}`);
  console.log(`  member: ${MEMBER_EMAIL} / ${MEMBER_PASS}`);
  console.log("Change both passwords after first login. Add your assets and goals with real figures in the app.");
}
